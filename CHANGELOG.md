...
- **ActiveMQ Scaler**: Allow `managementEndpoint` to be provided through `TriggerAuthentication` or a resolved environment variable, so the broker console address can come from the same Secret as the credentials ([#8129](https://github.com/kedacore/keda/pull/8129))
