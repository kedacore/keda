...
- **General**: Fix external push scalers dropping activations after a discovery cycle persists an `ExternalMetricNames` list that transiently omits their trigger, by resolving the push scaler's metric name directly from the scaler when the persisted status doesn't have it ([#8168](https://github.com/kedacore/keda/issues/8168))
