...
- **Prometheus Scaler**: Add support for `authModes: oauth`, authenticating queries with an OAuth2 client credentials `TriggerAuthentication`. The access token is acquired, cached and refreshed by the scaler's HTTP transport, so it is reused across polling intervals instead of being fetched per query ([#8064](https://github.com/kedacore/keda/pull/8064))
