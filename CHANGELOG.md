...
- **General**: Fix `ScaledObject` scale target resolution picking a different API group whose name starts with the requested one (for example `apps/v1` `StatefulSet` resolving to `statefulsets.apps.kruise.io` when the OpenKruise CRDs are installed), which made the `/scale` lookup fail with `not found` ([#8182](https://github.com/kedacore/keda/issues/8182))
