...
- **General**: Cap each scalers-cache reader at a per-reader budget derived from `globalHTTPTimeout` so `ScalersCache.Close` cannot block indefinitely on a wedged scaler reader, and release the global `scalerCachesLock` before draining scalers during `ClearScalersCache` to prevent one bad scaler from cascading into stuck `ScaledObject` finalizers and `<unknown>` HPA metrics across the operator ([#7574](https://github.com/kedacore/keda/issues/7574))
