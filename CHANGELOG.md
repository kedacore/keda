...
- **General**: Stop rejecting pre-existing `TriggerAuthentication`/`ClusterTriggerAuthentication` resources with `podIdentity.identityId: ""` at reconcile time; the operator no longer validates this field (validation stays in the admission webhook, which blocks *new* bad input). Fixes a breaking change introduced in v2.12.0 that stopped scaling for users upgrading from v2.11 with stored empty-string `identityId` values ([#5109](https://github.com/kedacore/keda/issues/5109))
