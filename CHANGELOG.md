...
- **General**: Fix `ScaledObject`/`ScaledJob`/`TriggerAuthentication`/`ClusterTriggerAuthentication` finalizers getting stuck in `Terminating` during bulk deletion by retrying finalizer add/remove on `resourceVersion` conflicts instead of failing the whole reconcile ([#7950](https://github.com/kedacore/keda/issues/7950))
