...
- **Dynatrace Scaler**: Handle `NOT_STARTED` as a valid initial DQL query state; environments that queue requests before a worker picks them up (e.g. `*.apps.dynatracelabs.com`) no longer error with `unknown state NOT_STARTED` ([#7986](https://github.com/kedacore/keda/pull/7986))
