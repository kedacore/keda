...
- **Redis Scaler**: Add support for Microsoft Entra ID authentication via Azure Workload Identity (`podIdentity.provider: azure-workload`) for `redis`, `redis-cluster`, and `redis-streams` triggers against Azure Managed Redis, with automatic token refresh via `go-redis-entraid` ([#7888](https://github.com/kedacore/keda/issues/7888))
