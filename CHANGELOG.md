...
- **General**: Grant `events` on the core API group in the operator ClusterRole, which client-go's event broadcaster also writes to, so leader-election and `ScaledObject` events are no longer rejected ([#7922](https://github.com/kedacore/keda/issues/7922))
