...
- **External Scaler**: Release the pooled gRPC connection when the last scaler using it is closed, so the connection, its pool entry and its watcher goroutine are no longer retained for the lifetime of the process ([#8192](https://github.com/kedacore/keda/issues/8192))
