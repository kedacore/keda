...
- **General**: Only refresh `ScaledObject.Status.LastActiveTime` at the moment triggers transition from active to inactive, instead of on every steady-state active poll, so the redundant status patch (and full-object etcd write) on every polling interval for active `ScaledObjects` is eliminated ([#7998](https://github.com/kedacore/keda/issues/7998))
