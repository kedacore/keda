...
const testNamespace = 'metrics-api-test'
const maxReplicaCount = 2
const endpoint = `http://metrics-entrypoint.${testNamespace}.svc.cluster.local:8080/api/value`
...
test.after.always.cb('clean up metrics-api deployment', (t) => {
  const resources = ['scaledobject.keda.sh/test-scaledobject']
  ...
  template:
    spec:
      ...
      - name: curl-client
        ...
        command: ["curl", "-X", "POST", "{{METRIC_SERVER_ENDPOINT}}/{{VALUE}}"]
      restartPolicy: Never`
