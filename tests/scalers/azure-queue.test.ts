...
test.serial.cb('Deployment should scale to 4 with 10,000 messages on the queue then back to 0', t => {
    // add 10,000 messages
    ...
    queueSvc.createQueueIfNotExists('queue-name', err => {
        ...
        async.mapLimit(Array(10000).keys(), 200, (n, cb) => queueSvc.createMessage('queue-name', `test ${n}`, cb), () => {
            let replicaCount = '0';
