...
import (
	...
	helper "github.com/kedacore/keda/v2/tests/scalers/openstack_metrics"
)
...
func TestScaler(t *testing.T) {
	require.NotEmpty(t, userID, "OPENSTACK_USER_ID env variable is required for OpenStack metrics test")
	require.NotEmpty(t, password, "OPENSTACK_PASSWORD env variable is required for OpenStack metrics test")
	require.NotEmpty(t, projectID, "OPENSTACK_PROJECT_ID env variable is required for OpenStack metrics test")
	...
	t.Cleanup(func() {
		...
		helper.DeleteMetric(t, metricsClient, metricsURL, metricID)
	})
...
func testActivation(t *testing.T, kc *kubernetes.Clientset, client openstack.Client, metricsURL, metricID string) {
	...
	helper.PostMeasure(t, client, metricsURL, metricID, 2)
...
func testScaleOut(t *testing.T, kc *kubernetes.Clientset, client openstack.Client, metricsURL, metricID string) {
	...
	helper.PostMeasure(t, client, metricsURL, metricID, 10)

	assert.True(t,
		WaitForDeploymentReplicaReadyCount(t, kc, deploymentName, testNamespace, maxReplicaCount, 60, 3),
...
func testScaleIn(t *testing.T, kc *kubernetes.Clientset, client openstack.Client, metricsURL, metricID string) {
	...
	helper.PostMeasure(t, client, metricsURL, metricID, 0)

	assert.True(t,
		WaitForDeploymentReplicaReadyCount(t, kc, deploymentName, testNamespace, minReplicaCount, 60, 5),
