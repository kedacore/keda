...
package azure_mssql_aad_azuresql_test
...
const (
	testName = "azure-mssql-aad-azuresql-test"
)
...
var (
	...
	azureIdentityAppID        = os.Getenv("TF_AZURE_IDENTITY_1_APP_ID")

	// The workload keeps using SQL authentication. Only the scaler is asked to
	...
	// The azuresql driver resolves this itself, using the federated token that
	// workload identity projects into the KEDA operator pod.
	azureMSSQLEntraConnectionString = fmt.Sprintf("Server=%s;Database=%s;fedauth=ActiveDirectoryWorkloadIdentity;user id=%s;Encrypt=true;TrustServerCertificate=false;",
		azureMSSQLFQDN, azureMSSQLDatabase, azureIdentityAppID)
...
type templateData struct {
	...
	AzureMSSQLEntraConnectionString string
	MinReplicaCount                 int
	MaxReplicaCount                 int
}
...
stringData:
  ...
  mssql-entra-connection-string: {{.AzureMSSQLEntraConnectionString}}
`
...
spec:
  ...
  - parameter: connectionString
    name: {{.SecretName}}
    key: mssql-entra-connection-string
`
...
func TestAzureMSSQLAzureSQLDriverScaler(t *testing.T) {
	if azureMSSQLFQDN == "" || azureMSSQLAdminUsername == "" ||
		...
		azureIdentityAppID == "" {
		t.Skip("Skipping: Azure MSSQL azuresql driver test requires TF_AZURE_SQL_SERVER_* and TF_AZURE_IDENTITY_1_APP_ID env vars")
	}
...
var data = templateData{
	...
	AzureMSSQLEntraConnectionString: azureMSSQLEntraConnectionString,
}
