...
func testGRPCClientMetrics(t *testing.T, kc *kubernetes.Clientset, data templateData) {
	...
	families := waitForCollectorMetric(t, "keda_rpc_client_call_count_total", func(family *prommodel.MetricFamily) bool {
		return slices.ContainsFunc(family.GetMetric(), func(metric *prommodel.Metric) bool {
			return matchesGRPCPullMetric(metric.GetLabel(), data, "GetMetrics") && metric.GetCounter().GetValue() >= 1
		})
	})
	for _, method := range []string{"IsActive", "GetMetricSpec"} {
		waitForCollectorMetric(t, "keda_rpc_client_call_count_total", func(family *prommodel.MetricFamily) bool {
			return slices.ContainsFunc(family.GetMetric(), func(metric *prommodel.Metric) bool {
	...
	durationFamily, ok := families["keda_rpc_client_call_duration_seconds"]
	assert.True(t, ok, "keda_rpc_client_call_duration_seconds not present")
	if ok {
		assert.True(t, slices.ContainsFunc(durationFamily.GetMetric(), func(metric *prommodel.Metric) bool {
			return matchesGRPCPullMetric(metric.GetLabel(), data, "GetMetrics") && metric.GetHistogram().GetSampleCount() > 0
	...
	waitForCollectorMetric(t, "keda_rpc_client_stream_message_count_total", func(family *prommodel.MetricFamily) bool {
		return slices.ContainsFunc(family.GetMetric(), func(metric *prommodel.Metric) bool {
			return matchesGRPCStreamMessage(metric.GetLabel(), data, "SENT") && metric.GetCounter().GetValue() >= 1
		})
	})
	...
	waitForCollectorMetric(t, "keda_rpc_client_stream_message_count_total", func(family *prommodel.MetricFamily) bool {
		return slices.ContainsFunc(family.GetMetric(), func(metric *prommodel.Metric) bool {
			return matchesGRPCStreamMessage(metric.GetLabel(), data, "RECEIVED") && metric.GetCounter().GetValue() >= 1
		})
	})
...
func matchesGRPCPullMetric(labels []*prommodel.LabelPair, data templateData, method string) bool {
	...
	return ExtractPrometheusLabelValue("namespace", labels) == data.TestNamespace &&
		...
		ExtractPrometheusLabelValue("rpc_method", labels) == "externalscaler.ExternalScaler/"+method &&
		ExtractPrometheusLabelValue("rpc_response_status_code", labels) == "OK" && metricNameMatches
}

func matchesGRPCStreamMessage(labels []*prommodel.LabelPair, data templateData, messageType string) bool {
	return matchesGRPCPushResource(labels, data) &&
		ExtractPrometheusLabelValue("rpc_method", labels) == "externalscaler.ExternalScaler/StreamIsActive" &&
		ExtractPrometheusLabelValue("rpc_message_type", labels) == messageType
}

func matchesGRPCStreamLifecycle(labels []*prommodel.LabelPair, data templateData) bool {
	return matchesGRPCPushResource(labels, data) &&
		ExtractPrometheusLabelValue("rpc_method", labels) == "externalscaler.ExternalScaler/StreamIsActive" &&
		ExtractPrometheusLabelValue("rpc_response_status_code", labels) == "CANCELLED"
}
