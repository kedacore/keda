...
var _ = Describe("ScaledObjectController", func() {
	...
	It("emits event when cooldownPeriod is configured with non-zero minReplicaCount", func() {
		deploymentName := "cooldown-with-minreplicas"
		soName := "so-" + deploymentName
		...
		// Create the ScaledObject with cooldownPeriod and minReplicaCount > 0
		so := &kedav1alpha1.ScaledObject{
			ObjectMeta: metav1.ObjectMeta{Name: soName, Namespace: "default"},
			Spec: kedav1alpha1.ScaledObjectSpec{
				ScaleTargetRef: &kedav1alpha1.ScaleTarget{
	...
	It("emits event when pollingInterval is configured with non-zero minReplicaCount", func() {
		...
		// Create the ScaledObject with pollingInterval and minReplicaCount > 0
		so := &kedav1alpha1.ScaledObject{
			ObjectMeta: metav1.ObjectMeta{Name: soName, Namespace: "default"},
			Spec: kedav1alpha1.ScaledObjectSpec{
				ScaleTargetRef: &kedav1alpha1.ScaleTarget{
	...
	It("emits both events when cooldownPeriod and pollingInterval are configured with non-zero minReplicaCount", func() {
		deploymentName := "both-with-minreplicas"
		soName := "so-" + deploymentName
		...
		// Create the ScaledObject with both cooldownPeriod and pollingInterval with minReplicaCount > 0
		so := &kedav1alpha1.ScaledObject{
			ObjectMeta: metav1.ObjectMeta{Name: soName, Namespace: "default"},
			Spec: kedav1alpha1.ScaledObjectSpec{
				ScaleTargetRef: &kedav1alpha1.ScaleTarget{
		...
		// Check that both informational events were created
		eventList := &corev1.EventList{}
		foundCooldownEvent := false
		...
		Eventually(func() bool {
			...
			for _, event := range eventList.Items {
				if event.InvolvedObject.Name == soName &&
					...
					if strings.Contains(event.Message, "PollingInterval is configured but minReplicaCount is 1") {
						foundPollingEvent = true
					}
				}
			}
			return foundCooldownEvent && foundPollingEvent
		}, 30*time.Second, 2*time.Second).Should(BeTrue())
	})

	It("does not emit event when minReplicaCount is 0", func() {
		...
		// Create the ScaledObject with cooldownPeriod and pollingInterval with minReplicaCount = 0
		so := &kedav1alpha1.ScaledObject{
			ObjectMeta: metav1.ObjectMeta{Name: soName, Namespace: "default"},
			Spec: kedav1alpha1.ScaledObjectSpec{
				ScaleTargetRef: &kedav1alpha1.ScaleTarget{
