...
func TestGetPausedReplicaCount(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			scaledObject: func() *kedav1alpha1.ScaledObject {
		...
		},
		{
			...
			scaledObject: func() *kedav1alpha1.ScaledObject {
				...
				so.Annotations = map[string]string{
					kedav1alpha1.PausedReplicasAnnotation: "5",
				}
				return so
			}(),
			wantNil:   false,
		...
		},
		{
			...
			scaledObject: func() *kedav1alpha1.ScaledObject {
				so := &kedav1alpha1.ScaledObject{}
				so.Annotations = map[string]string{
					kedav1alpha1.PausedReplicasAnnotation: "not-a-number",
				}
				return so
			}(),
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := GetPausedReplicaCount(tt.scaledObject)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
