...
func TestAwsParameterStoreHandler_InitializeUsingStaticCredentials(t *testing.T) {
	...
	awsParameterStoreHandler := &AwsParameterStoreHandler{
		parameterStore: &kedav1alpha1.AwsParameterStore{
			Region: expectedAwsRegion,
			Credentials: &kedav1alpha1.AwsParameterStoreCredentials{
				AccessKey: &kedav1alpha1.AwsParameterStoreValue{
				...
				AccessSecretKey: &kedav1alpha1.AwsParameterStoreValue{
					ValueFrom: kedav1alpha1.ValueFromSecret{
						SecretKeyRef: kedav1alpha1.SecretKeyRef{
							Name: "mocked-access-key-secret",
							Key:  "mocked-access-key-key",
	...
	err := awsParameterStoreHandler.Initialize(context.Background(), mockClient, logr.Discard(), "mocked-trigger-namespace", nil, &corev1.PodSpec{})

	assert.NoError(t, err)
	assert.Equal(t, expectedAwsRegion, awsParameterStoreHandler.parameterStore.Region)
}

func TestAwsParameterStoreHandler_InitializeUsingPodIdentity(t *testing.T) {
	tests := []struct {
	...
	for _, tc := range tests {
		...
		err := awsParameterStoreHandler.Initialize(context.Background(), nil, logr.Discard(), "mocked-trigger-namespace", nil, &corev1.PodSpec{})
