...
// alive.
func (h *scaleHandler) watchMetricSpecUpdates(ctx context.Context, scaledObjectName, scaledObjectNamespace string, triggerIndex int, streamer scalers.MetricSpecStreamer) {
	...
	for {
		...
		case specs, ok := <-streamer.MetricSpecChan():
			...
			if scalersCache.UpdateMetricSpecForScaler(triggerIndex, specs) {
	...
	for i, ps := range scalersCache.GetPushScalers() {
		...
		if so, isScaledObject := scalableObject.(*kedav1alpha1.ScaledObject); isScaledObject {
			if streamer, ok := ps.Scaler.(scalers.MetricSpecStreamer); ok {
				go h.watchMetricSpecUpdates(ctx, so.Name, so.Namespace, ps.TriggerIndex, streamer)
			}
		}
