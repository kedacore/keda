...
var testAzurePipelinesMetadata = []parseAzurePipelinesMetadataTestData{
	...
	// missing poolName
	{"missing poolName", map[string]string{"organizationURLFromEnv": "AZP_URL", "personalAccessTokenFromEnv": "AZP_TOKEN", "poolID": "1", "targetPipelinesQueueLength": "1"}, true, testAzurePipelinesResolvedEnv, map[string]string{}},
	...
	// setting unsafeSsl
	{"missing unsafeSsl", map[string]string{"organizationURLFromEnv": "AZP_URL", "personalAccessTokenFromEnv": "AZP_TOKEN", "poolID": "1", "targetPipelinesQueueLength": "1", "unsafeSsl": "false"}, true, testAzurePipelinesResolvedEnv, map[string]string{}},
}
...
func TestParseAzurePipelinesMetadata(t *testing.T) {
	for _, testData := range testAzurePipelinesMetadata {
		t.Run(testData.testName, func(t *testing.T) {
			...
			metadata, err := parseAzurePipelinesMetadata(&ScalerConfig{TriggerMetadata: testData.metadata, ResolvedEnv: testData.resolvedEnv, AuthParams: testData.authParams})
			if err != nil && !testData.isError {
				t.Error("Expected parsing success but got error", err)
			}

			err = validateAzurePipelinesMetadata(context.TODO(), metadata, http.DefaultClient)
			if err != nil && !testData.isError {
				t.Error("Expected validation success but got error", err)
			}
...
func TestValidateAzurePipelinesPool(t *testing.T) {
	for _, testData := range testValidateAzurePipelinesPoolData {
		t.Run(testData.testName, func(t *testing.T) {
			...
			metadata, err := parseAzurePipelinesMetadata(&ScalerConfig{TriggerMetadata: testData.metadata, ResolvedEnv: nil, AuthParams: authParams})
			if err != nil && !testData.isError {
				t.Error("Expected parsing success but got error", err)
			}
...
func TestAzurePipelinesGetMetricSpecForScaling(t *testing.T) {
	for _, testData := range azurePipelinesMetricIdentifiers {
		...
		meta, err := parseAzurePipelinesMetadata(&ScalerConfig{TriggerMetadata: metadata, ResolvedEnv: nil, AuthParams: authParams, ScalerIndex: testData.scalerIndex})
		if err != nil {
...
func TestAzurePipelineMatchedUnsafeSslAgent(t *testing.T) {
	var apiStub = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write(buildLoadJSON())
	}))
	...
	meta, err := parseAzurePipelinesMetadata(&ScalerConfig{TriggerMetadata: metadata, ResolvedEnv: nil, AuthParams: authParams})
	if err != nil {
		t.Fatal("Could not parse metadata:", err)
	}

	err = validateAzurePipelinesMetadata(context.TODO(), meta, http.DefaultClient)
	if err != nil {
		t.Error("Could not validation metadata:", err)
	}

	if meta.unsafeSsl != true {
		t.Fatal("Could not validate unsafeSsl parsing in metadata")
	}
}
