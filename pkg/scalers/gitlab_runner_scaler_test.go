...
func TestParseGitLabRunnerMetadata(t *testing.T) {
	...
	if meta.GitLabAPIURL.String() != "https://gitlab.com" {
...
func TestGetPipelineQueueLength_MaxPagesExceeded(t *testing.T) {
	...
	// pagination starts with page 1, not 0
	assert.Equal(t, int64(maxGitlabAPIPageCount-1), int64(serverCallCount))
	assert.Equal(t, int64(maxGitlabAPIPageCount-1), count)
}
