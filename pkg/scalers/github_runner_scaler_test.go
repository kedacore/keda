...
func TestGithubRunnerPruneCompletedJobsDropsFinishedRuns(t *testing.T) {
	...
	s.etags = map[string]string{
		s.jobsAPIURL("repo", 100, 1): "etag-100", // still active, kept
		s.jobsAPIURL("repo", 200, 1): "etag-200", // completed, dropped
		s.jobsAPIURL("repo", 200, 2): "etag-200", // completed, dropped
	}
	...
	if _, ok := s.etags[s.jobsAPIURL("repo", 200, 2)]; ok {
		t.Errorf("etags still contains completed run 200's jobs URL after pruneCompletedJobs")
	}
	if _, ok := s.etags[s.jobsAPIURL("repo", 100, 1)]; !ok {
...
func TestGithubRunnerPruneCachesBoundsMaps(t *testing.T) {
	...
	s := githubRunnerScaler{
		metadata:         &githubRunnerMetadata{},
		previousJobPages: make(map[jobCacheKey][]Job, overflow),
	...
	for i := range overflow {
		...
		// Two runs per repo, so the (repo, runID, page) tuple count exceeds the repo
		// count and genuinely tests the total-entries bound, not just len(map).
		s.previousJobPages[jobCacheKey{repo: repo, runID: int64(i), page: 1}] = nil
		s.previousJobPages[jobCacheKey{repo: repo, runID: int64(i + overflow), page: 1}] = nil
...
func TestGetWorkflowRunJobs_SecondPageChangedWhileFirstPageNotModified(t *testing.T) {
	...
	page1URL := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%d/jobs?per_page=%d",
		meta.GithubAPIURL, meta.Owner, repo, int64(30433642), githubJobsPerPage)
	...
	s := githubRunnerScaler{
		...
		previousJobPages: map[jobCacheKey][]Job{
			{repo: repo, runID: int64(30433642), page: 1}: stalePage1Jobs,
			{repo: repo, runID: int64(30433642), page: 2}: stalePage2Jobs,
		},
		previousWfrs: map[string]map[string]*WorkflowRuns{},
	}

	jobs, err := s.getWorkflowRunJobs(context.Background(), 30433642, repo)
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
