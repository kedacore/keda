...
func TestGetQueueInfoWithRegex(t *testing.T) {
	for _, testData := range testRegexQueueInfoTestData {
		var apiStub = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			expectedPath := "/api/queues?use_regex=true&pagination=true&name=%5Eevaluate_trials%24&page=1&page_size=100"
			if r.RequestURI != expectedPath {
				t.Error("Expect request path to =", expectedPath, "but it is", r.RequestURI)
			}
...
func TestGetQueueInfoNavigation(t *testing.T) {
	for _, testData := range testRegexQueueInfoNavigationTestData {
		currentPage := 0
		var apiStub = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			response := testData.responses[currentPage]
			currentPage++
			expectedPath := fmt.Sprintf("/api/queues?use_regex=true&pagination=true&name=evaluate_trials&page=%d&page_size=100", currentPage)
			if r.RequestURI != expectedPath {
				t.Error("Expect request path to =", expectedPath, "but it is", r.RequestURI)
			}
		...
		assert.Equal(t, currentPage, len(testData.responses))
	}
}
